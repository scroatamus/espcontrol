...
void SendspinMediaSource::on_stream_start() {
  ...
  // Recover from a stale pending flag. If no source is actively playing, a new
  // stream start must be allowed to re-request play_uri.
  if (this->pending_start_ && this->get_state() == media_source::MediaSourceState::IDLE) {
    ESP_LOGW(TAG, "Resetting stale pending_start flag while IDLE");
    this->pending_start_ = false;
  }
