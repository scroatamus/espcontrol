...
|---|---|---|---|---|
| `ha_reconnect_recovery_test.py` artwork retry and reconnect source assertions | Repeated unchanged downloads or lost track-change refreshes after retries/reconnects | Executable production request, response, timeout, and reconnect functions with simulated HA/LVGL I/O | Drop the unfinished refresh flag; force companion retries; force every reconnect | Replaced by seven behavioral scenarios; all three mutations fail their matching download-count checks |
